...
var letters = nodes.reduce( function (letters, node) {
  while(node.nodeValue) {
    var next = node.splitText(1)
...
var animation1 = 'jsoxconf'.split('')
  .reduce(function (timeline, letter, i) {
...
var animation2 = ['jjss','ooxx','conf']
  ...
  .reduce( function (timeline, letters, i) {
    var c  =  i % 2 ? 'aquamarine' : '#f08'
...
// random
var animation3 = Array.from({length:40}, function ( _ ) { return [
  Math.random() * 20,
...
var animation4 = Array.from({length:40}, function (_,i) { return [
  Math.random() * 120,
  i % 8,
  random(['👏','🎉','💯','💕','💥'])
]; })
